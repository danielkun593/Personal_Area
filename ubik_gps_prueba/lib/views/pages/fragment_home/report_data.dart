...
class ReportsData extends StatefulWidget {
  const ReportsData({Key key, @required this.token}) : super(key: key);
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        body: TabBarView(
          ...
          children: [
            allReport(token: widget.token),
            onlineReport(token: widget.token),
            offlineReport(token: widget.token),
            otherReport(token: widget.token)
          ],
        ),
      ),
    );
